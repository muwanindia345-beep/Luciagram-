...
export default function Profile() {
  ...
  useEffect(() => {
    API.get("/posts/feed").then(r => {
      setPosts(r.data.filter(p => p.userId === user?.id));
    }).catch(() => {});
  }, [user]);
    ...
    <div style={{background:"#0a0a0f",minHeight:"100vh",color:"white",paddingBottom:"70px"}}>
      ...
      <div style={{background:"#0a0a0f",borderBottom:"1px solid #1e1e2e",padding:"0.75rem 1rem",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",gap:"1rem",fontSize:"1.3rem"}}>
          <span style={{cursor:"pointer"}} onClick={handleLogout}>⚙️</span>
        </div>
      </div>
      ...
      <div style={{padding:"1.5rem 1rem"}}>
        <div style={{display:"flex",alignItems:"center",gap:"1.5rem",marginBottom:"1rem"}}>
          <div style={{width:"86px",height:"86px",borderRadius:"50%",background:"linear-gradient(135deg,#7c3aed,#db2777)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2rem",fontWeight:"bold",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:"1.5rem",flex:1,justifyContent:"space-around"}}>
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontWeight:"bold",fontSize:"1.2rem"}}>{followersCount}</div>
              <div style={{color:"#888",fontSize:"0.8rem"}}>Followers</div>
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontWeight:"bold",fontSize:"1.2rem"}}>{followingCount}</div>
        ...
        <div style={{marginBottom:"1rem"}}>
          ...
          <div style={{color:"#888",fontSize:"0.85rem",marginTop:"0.2rem"}}>{user?.bio || "No bio yet"}</div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"2px",padding:"2px"}}>
        {posts.length === 0 ? (
          ...
          <div key={p.id||i} style={{aspectRatio:"1",overflow:"hidden",cursor:"pointer",position:"relative"}}>
            {p.mediaUrl ? (
