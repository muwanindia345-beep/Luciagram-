  ...
  const handlePost = async () => {
    ...
    try {
      if (type === "post") {
        ...
        await API.post("/stories", { mediaBase64: image, mediaType: "image" });
      }
      navigate("/");
    } catch(err) {
  ...
  return (
    <div style={{background:"#0a0a0f",minHeight:"100vh",color:"white"}}>
      ...
      <div style={{display:"flex",margin:"1rem",background:"#1e1e2e",borderRadius:"12px",padding:"4px"}}>
        <button onClick={()=>setType("post")} style={{flex:1,padding:"0.6rem",border:"none",borderRadius:"10px",background:type==="post"?"linear-gradient(135deg,#7c3aed,#db2777)":"transparent",color:"white",cursor:"pointer",fontWeight:"bold",fontSize:"0.9rem"}}>📸 Post</button>
        <button onClick={()=>setType("story")} style={{flex:1,padding:"0.6rem",border:"none",borderRadius:"10px",background:type==="story"?"linear-gradient(135deg,#7c3aed,#db2777)":"transparent",color:"white",cursor:"pointer",fontWeight:"bold",fontSize:"0.9rem"}}>⏱️ Story (24h)</button>
      </div>
      ...
      <div style={{maxWidth:"600px",margin:"0 auto",padding:"0 1rem"}}>
        {!preview ? (
          <label style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"300px",border:"2px dashed #2a2a3a",borderRadius:"16px",cursor:"pointer",gap:"1rem"}}>
            <div style={{fontSize:"3rem"}}>{type==="story"?"⏱️":"📸"}</div>
            <p style={{color:"#888",margin:0}}>{type==="story"?"Tap to select photo for story":"Tap to select photo or video"}</p>
            {type==="story" && <p style={{color:"#7c3aed",margin:0,fontSize:"0.85rem"}}>⏰ Auto-deletes after 24 hours</p>}
          ...
          <div style={{position:"relative"}}>
            ...
            {type==="story" && (
              <div style={{position:"absolute",top:"0.75rem",left:"0.75rem",background:"rgba(0,0,0,0.7)",borderRadius:"20px",padding:"0.3rem 0.75rem",display:"flex",alignItems:"center",gap:"0.4rem"}}>
                <span style={{fontSize:"0.8rem"}}>⏰</span>
                <span style={{color:"white",fontSize:"0.8rem",fontWeight:"bold"}}>24h Story</span>
              </div>
            )}
        ...
        {type === "story" && preview && (
          <div style={{background:"#13131a",borderRadius:"12px",padding:"1rem",marginTop:"1rem",display:"flex",alignItems:"center",gap:"0.75rem"}}>
            ...
            <div>
              ...
              <div style={{color:"#888",fontSize:"0.8rem"}}>This story will automatically disappear after 24 hours</div>
            </div>
          </div>
        )}
